    ...
    public class CheckoutController : ApiControllerBase
    {
        ...
        public async Task<IActionResult> SetPaymentProvider(CheckoutViewModel model)
        {
            ...
            catch (Exception ex)
            {
                ...
                ModelState.AddModelError("general", "checkout.setpaymenterror".AsWebsiteText());
                return BadRequest(ModelState);
            }
        }
        ...
        public IActionResult SetCampaignCode(CheckoutViewModel model)
        {
            try
            {
                //if (_checkoutPageService.SetCampaignCode(model.CampaignCode ?? string.Empty))
                //{
                //    if (model.PaymentWidget != null)
                //    {
                ...
                //ModelState.AddModelError(nameof(CheckoutViewModel.CampaignCode), "checkout.campaigncodeinvalid".AsWebsiteText());
                //return BadRequest(ModelState);
            ...
            {
                _logger.LogError(ex, "Error when setting campaign code {CampaignCode}", model.CampaignCode);
                ModelState.AddModelError("general", "checkout.setcampaigncodeerror".AsWebsiteText());
                return BadRequest(ModelState);
            }
        }
        ...
        private async Task AddOrUpdateCustomerInfoAsync(CheckoutViewModel model, CartContext cartContext)
        {
            var addOrUpdateCustomerInfoArgs = new AddOrUpdateCustomerInfoArgs
            {
                CustomerInfo = new CustomerInfo
                {
                    ...
                    CustomerType = cartContext.Cart.Order.CustomerInfo?.CustomerType ?? CustomerType.PrivatePerson,
        ...
        private async Task RegisterNewUserAsync(CartContext cartContext)
        {
            ...
            var template = _templateService.Get<PersonFieldTemplate>(typeof(CustomerArea), "B2CPersonTemplate");
